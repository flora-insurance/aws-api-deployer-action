...
module.exports = class ApiGateway {
  ...
  async exportApi(id, options = {}) {
    const params = {
      restApiId: id,
      exportType: options.exportApi || 'swagger',
  ...
  async updateWebAcl(restApiId, domainName, stageName = 'default') {
    ...
    const webAclArn = ListWebACLsResponse.WebACLs.find(entry => entry.Name === config.webAcl[domainName] ? config.webAcl[domainName] : 'standard').ARN || ''
